...
class PriceQuotationChats extends Component
{
    public $jobs = [];
    ...
    public function mount(PriceQuotation $price_quotation)
    {
        ...
        $this->items = $price_quotation->items ?? setting('items');
        $this->items_en = $price_quotation->items_en ?? setting('items_en');
        $this->items_two = $price_quotation->items_two ?? setting('items_two');
        $this->items_two_en = $price_quotation->items_two_en ?? setting('items_two_en');

        $default_items = config()->get('price-quotation.items');
        $default_bands = config()->get('price-quotation.bands');
        foreach ($default_items as $key =>$value) {
            $this->$key = $price_quotation->$key ?? $value;
        }
        foreach ($default_bands as $key => $value){
            $ar_key = 'band_' . $key.'_ar';
            $en_key = 'band_' . $key.'_en';

            $this->$ar_key = $price_quotation->$ar_key ?? $value;
            $this->$en_key = $price_quotation->$en_key ?? $value;
        }
    ...
    private function calculateBasicSalary($key)
    {
        ...
        $this->jobs[$key]['total_salary'] = ceil($total);
    }
    ...
    private function calculateBenefits(string $key): void
    {
        ...
        $this->jobs[$key]['vacation_ticket_cost'] = ceil($this->calculateVacationTicketCost($salary, $months));
        $this->jobs[$key]['end_of_service_cost'] = ceil($this->calculateEndOfServiceCost($totalSalary));
        $this->jobs[$key]['vacation_cost'] = ceil($this->calculateVacationCost($key, $months));
        $this->jobs[$key]['end_of_service_ticket_cost'] = $months ? ceil(1000 / $months) : 0;
    }
    ...
    private function calculateVacationCost(string $key, int $months): float
    {
        ...
        return ($salary && $vacationDays) ? round(($salary / 30) * ($vacationDays / $months), 2) : 0;
    }
    ...
    private function calculateTotalsAndProfits(string $key): void
    {
        ...
        $totalCosts = ceil((float)$this->jobs[$key]['total_indirect_cost'] + $this->jobs[$key]['total_salary']);
        $netProfit = ceil($sellingCost) - $totalCosts;
    ...
    private function calculateMultipleEmployeeTotals(string $key, int $number): void
    {
        ...
        foreach ($fields as $source => $target) {
            $this->jobs[$key][$target] =$this->jobs[$key][$source] * $number;
        }
    }
    ...
    public function submit(): void
    {
        $this->price_quotation->update([
            ...
            'head_ar' => $this->head_ar,
            'head_en' => $this->head_en,
            'head ' => $this->head,

            'band_ar_32' => $this->band_ar_32,
            'band_en_32' => $this->band_en_32,
        ]);
        $this->price_quotation->jobs()->delete();
        $this->price_quotation->jobs()->createMany($this->jobs);
        //        $this->dispatch('alert', message: 'تم التعديل بنجاح', type: 'success');
        $this->redirect(route('admin.prices.show', $this->price_quotation->id));
    }
    ...
    public function render()
    {
        $messages = PriceQuotationMessage::where('price_quotation_id', $this->price_quotation->id)
            ->get();
        return view('livewire.price-quotation-chats', compact('messages'))->extends('admin.layouts.admin')->section('content');
    }
}
